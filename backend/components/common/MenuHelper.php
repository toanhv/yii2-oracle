...
class MenuHelper {
    ...
    public static function getAssignedMenu($userId, $root = null, $callback = null, $refresh = false) {
        ...
        $menus = Menu::find()->orderBy('PRIORITY')->asArray()->all();
        $key = [__METHOD__, $userId, $manager->defaultRoles];
        $cache = null; //$config->cache;
        ...
        if ($refresh || $cache === null || ($assigned = $cache->get($key)) === false) {
            ...
            foreach ($manager->getRolesByUser($userId) as $role) {
                foreach ($manager->getPermissionsByRole($role->name) as $name => $value) {
                    if ($name[0] === '/') {
                        if (substr($name, -2) === '/*') {
                            $name = substr($name, 0, -1);
                        }
            ...
            if (count($filter2)) {
                $assigneds = $query->where(['route' => $filter2])->all();
                foreach ($assigneds as $item) {
                    $assigned[] = $item['ID'];
                }
            }
            if (count($filter1)) {
                ...
                foreach ($filter1 as $filter) {
                    $assigneds = $query->params([':filter' => $filter])->all();
                    foreach ($assigneds as $item) {
                        $assigned2[] = $item['ID'];
                    }
                    $assigned = array_merge($assigned, $assigned2);
                }
            }
