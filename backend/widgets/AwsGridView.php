...
use Yii;
use Closure;
use backend\overrides\grid\AwsColumn;
use backend\overrides\grid\AwsDataColumn;
use backend\overrides\grid\AwsGridViewAsset;
 ...
 * @since 2.0
 */
class AwsGridView extends AwsBaseListView
{
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $options = ['class' => 'dataTables_wrapper dataTables_extended_wrapper no-footer'];
    /**
     * @var array the HTML attributes for the table header row.
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $headerRowOptions = ['class' => 'heading', 'role' => 'row'];
    /**
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $filterRowOptions = ['class' => 'filters', 'role' => 'row'];
    /**
    ...
    /**
     * @var string Class name for odd rows style
     */
    public $oddRowClass = 'odd';
    /**
     * @var string Class name for even rows style
     */
    public $evenRowClass = 'even';
    ...
    public function run()
    {
        ...
        $view = $this->getView();
        AwsGridViewAsset::register($view);
    ...
    protected function getClientOptions()
    {
        ...
        $filterSelector = "#$id input, #$id select, select[name=\"per-page\"], input.aws-pager ";
    ...
    public function renderItems()
    {
        ...
        return '<div class="table-scrollable">' . Html::tag('table', implode("\n", $content), $this->tableOptions) . '</div>';
    }
    ...
    public function renderTableBody()
    {
        ...
        foreach ($models as $index => $model) {
            ...
            $rows[] = $this->renderTableRow($model, $key, $index, $odd);
     ...
     * @param bool $odd odd or even row (for styles)
     * @return string the rendering result
     */
    public function renderTableRow($model, $key, $index, $odd = true)
    {
        $cells = [];
        /* @var $column AwsColumn */
        foreach ($this->columns as $column) {
        ...
        if ($odd) {
            $options['class'] = isset($options['class']) ?  $options['class'] . ' ' . $this->oddRowClass : $this->oddRowClass;
        } else {
            $options['class'] = isset($options['class']) ?  $options['class'] . ' ' . $this->oddRowClass : $this->evenRowClass;
        }
        ...
        foreach ($this->columns as $i => $column) {
            if (is_string($column)) {
                ...
                $column = Yii::createObject(array_merge([
                    'class' => $this->dataColumnClass ?: AwsDataColumn::className(),
                    'grid' => $this,
                ], $column));
            }
            if (!$column->visible) {
     ...
     * @return AwsDataColumn the column instance
