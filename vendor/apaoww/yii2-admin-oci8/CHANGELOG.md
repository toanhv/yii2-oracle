...
- Enh: Custom side menu via `apaoww\AdminOci8\Module::items` (mdmunir).
...
- Chg: Can customize return of `apaoww\AdminOci8\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir).
...
- Chg: Remove `apaoww\AdminOci8\components\BizRule` (mdmunir).
...
- Chg: Remove `apaoww\AdminOci8\Module::allowActions`. Set access control directly with `apaoww\AdminOci8\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `apaoww\AdminOci8\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
