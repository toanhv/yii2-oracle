...
class Schema extends \yii\db\Schema {
    ...
    public function quoteSimpleTableName($name) {
        //ced: force uppercase (todo: check if we need to take care about config pdo attr)
        return strpos($name, '"') !== false ? $name : strtoupper($name);
    }
    ...
    public function quoteSimpleColumnName($name) {
        //ced: force uppercase (todo: check if we need to take care about config pdo attr)
        return strpos($name, '"') !== false || $name === '*' ? $name : strtoupper($name);
    }
    ...
    protected function resolveTableNames($table, $name) {
        ...
        if (isset($parts[1])) {
            ...
            $table->name = strtoupper($parts[1]); //ced (todo: take care about config pdo attr)
        } else {
            $table->schemaName = $this->defaultSchema;
            $table->name = strtoupper($name); //ced (todo: take care about config pdo attr)
        }
...
SQL;
    ...
    protected function createColumn($column) {
        ...
        //ced20150907: solve case troubles with oracle
        //(table et column names are in uppercase in oracle; attributs are in lowercase in yii)
        if ($this->db->slavePdo->getAttribute(\PDO::ATTR_CASE) === \PDO::CASE_LOWER) {
            ...
            if (stripos($column['DATA_DEFAULT'], 'timestamp') !== false) {
                ...
                if ($c->type === 'timestamp' && $defaultValue === 'CURRENT_TIMESTAMP') {
                    ...
                    if ($defaultValue !== null) {
                        if (($len = strlen($defaultValue)) > 2 && $defaultValue[0] === "'" && $defaultValue[$len - 1] === "'"
                        ) {
...
SQL;
        ...
        foreach ($command->queryAll() as $row) {
            //Bug #9415: Fixed regression in 2.0.6 where on Oracle DB `PDO::ATTR_CASE = PDO::CASE_UPPER` did not work anymore (cebe) //ced copy
            if ($this->db->slavePdo->getAttribute(\PDO::ATTR_CASE) === \PDO::CASE_LOWER) {
...
SQL;
    ...
    protected function extractColumnType($column, $dbType, $precision, $scale, $length) {
        ...
        if (strpos($dbType, 'FLOAT') !== false || strpos($dbType, 'DOUBLE') !== false) {
            ...
            //ced20150908: avoid error type de column for some real values...
